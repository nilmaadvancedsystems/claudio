...
começar: é o que deixa claro se esta execução está usando as regras de produção ou as de
um clone de teste.
...
⚠️ Se `<RAIZ_REGRAS>` **não** for a pasta de produção
(`G:\Meu Drive\CLAUDE FAVOR NÃO MEXER\_CLAUDIO_CONTROLE`) e `$1` for `PRODUCAO`, pare e
confirme com o usuário antes de qualquer escrita: é um clone de teste apontando para os
documentos reais dos clientes, e arquivaria pra valer com regra possivelmente não aprovada.
`SIMULACAO` a partir de clone é o uso esperado e não precisa de confirmação.
